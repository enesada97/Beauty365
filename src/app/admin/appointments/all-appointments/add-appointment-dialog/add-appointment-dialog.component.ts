...
export class AddAppointmentDialogComponent implements OnInit {
  ...
  createContactForm(): FormGroup {
    return this.fb.group({
      ...
      departmentId: [this.appointment.departmentId, [Validators.required]],
      ...
      surName: [this.appointment.surName, [Validators.required]],
  ...
  ngOnInit(): void {
    ...
    if (this.action == "edit") {
      ...
      this.depForDoctorsService
        .getById(this.appointment.doctorId)
        .subscribe((data) => (this.doctor = data));
    } else {
      this.departmentService.getList().subscribe((data) => {
        this.departments = data;
        if (this.departments[0]) {
          this.appointmentForm.controls["departmentId"].setValue(
            this.departments[0].id
          );
          this.depForDoctorsService
            .getDoctorListByDepId(this.departments[0].id)
            .subscribe((m) => {
              this.doctors = m;
              if (this.doctors[0]) {
      ...
      this.protocolTypeService.getList().subscribe((data) => {
        ...
        if (this.protocolTypes[0]) {
          this.appointmentForm.controls["protocolTypeId"].setValue(
  ...
  submit() {
    if (this.appointmentForm.valid) {
      ...
      if (this.appointment.id == 0) {
        this.patientService
          .getSearchedPatients(this.patient)
          .subscribe((res) => {
            if (res && res.length) {
                  ...
                  if (result.isConfirmed) {
                    ...
                    this.appointmentService
                      ...
                      swalWithBootstrapButtons.fire({
                        title: this.translate.instant('SavedAppointmentToAlreadyPatient'),
                        icon:"success",
                    ...
                    this.appointmentService
                      ...
                      .subscribe((data) => {
                        this.dialogRef.close(1);
                        this.sweetAlert.success(this.translate.instant('Added'));
                      });
                  }
                });
            } else {
            ...
            .subscribe((res) => {
              if (res && res.length) {
                ...
                swalWithBootstrapButtons
                  .fire({
                    title:
                    this.translate.instant('AlreadySavedPatientForThisPhoneNumberUpdateConfirm'),
                    showClass: {
                      popup: "animate__animated animate__fadeInDown",
                    },
                    hideClass: {
                  ...
                  .then((result) => {
                    if (result.isConfirmed) {
                      ...
                      swalWithBootstrapButtons.fire({
                        title: this.translate.instant('UpdatedAppointmentForAlreadyPatient'),
                        icon:"success",
                        confirmButtonText: this.translate.instant('OK')
                      });
                    } else {
  ...
  onSearchSubmit(): void {
    ...
    this.patient.identityNumber == null
      ? (this.patient.identityNumber = 0)
      : null;
  ...
  addAppointmentForSearch(row: Patient) {
    ...
    this.appointmentForm.controls["identityNumber"].setValue(
      row.identityNumber
    );
